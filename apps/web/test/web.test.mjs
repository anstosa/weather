...
test("dashboard separates current conditions from the historical logs route", () => {
  ...
  assert.match(windTrendsHtml, /data-trend-chart="windGustMps"[\s\S]*?<h2 class="trend-chart-title">Wind gust<\/h2>[\s\S]*?data-trend-metric-option="windGustMps" role="menuitemradio" aria-checked="true">Wind gust<\/button>/u);
