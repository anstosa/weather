...
enum WeatherWidgetFixtures {
    ...
    // select the debug host scenario
    static var active: WeatherWidgetFixture {
        #if DEBUG
        // honor only the fixed scheme values
        if let value = ProcessInfo.processInfo.environment["WEATHER_WIDGET_FIXTURE"],
           let scenario = WeatherWidgetScenario(rawValue: value) {
